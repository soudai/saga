...
## 作業
- `sg serve` と `sg doctor` の実装
- config loader の実装
- `slog` による構造化ログ初期化
- systemd readiness notify の導入

## 実装ステップ
1. `cobra` ベースで `sg` ルートコマンドと `serve`, `doctor`, `version` の最小サブコマンドを定義する。
2. `/etc/sg/config.yaml` とプロジェクトローカル override を見据えた config schema を決め、validation を実装する。
