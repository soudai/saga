...
本書は `saga` を WSL2 Ubuntu 上で `systemd` 管理の常駐サービスとして動かす際の運用要件をまとめる。
...
- `Codex CLI`, `git`, `saga` binary が Ubuntu 側にインストール済み

## 3. 推奨配置

- binary: `/usr/local/bin/saga`
- config: `/etc/saga/config.yaml`
- secrets: `/etc/saga/saga.env`
- state: `/var/lib/saga`
- runtime: `/run/saga`
- logs: `/var/log/saga`
...
- 起動失敗時は `journalctl -u saga -n 200`
- stale worktree は `saga doctor` と startup cleanup で検出
